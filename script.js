...
function drawFX() {
    ...
    if (formActive && formParticles.length > 0) {
        ...
        // Kalp oluşumu tamamlanınca final metni göster
        if (formProgress >= 1) {
...
// ===== Uzak mesafe → Final (kalp oluşumu başlar) =====
let ldTimer = null;
const ldObs = new MutationObserver(() => {
    ...
    if (ld.classList.contains('active')) {
        if (!ldTimer) {
            ldTimer = setTimeout(() => {
                goToSection(3);          // Final bölümüne geç
                startHeartFormation();    // Kalp oluşumunu başlat
                ldTimer = null;
            }, 5000);
        }
    } else {
...
function startHeartFormation() {
    ...
    formProgress = 0;
    createFormation();
    document.getElementById('final-text').style.opacity = '0'; // Metin başta gizli, kalp oluşunca görünür
}

// Başlangıçta intro aktif
goToSection(0);